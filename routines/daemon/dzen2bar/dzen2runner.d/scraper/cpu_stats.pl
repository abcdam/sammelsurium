...
sub get_update {
    ...
    my @delta           = map { $current_stats[$_] - $previous_stats->[$_] } 0 .. $#current_stats;
    my $freq_avg        = _get_avg_freq();
    my $avg_freq_entry  = $self->colorify_entry('ghz', $self->{color}{label}) . "@"
                        . $self->colorify_entry($freq_avg, $self->{color}{value});
    my @cpu_stat_values = map {
        $self->colorify_entry($_, $self->{color}{value})
        } _calculate_percentages(@delta);

    my @cpu_stat_labels = map {
        $self->colorify_entry("${_}", $self->{color}{label})
        } qw(usr nice sys idl iozz hirq sirq);

    $previous_stats = \@current_stats;

    return $avg_freq_entry . ' ' . join ' ', map {
        $cpu_stat_labels[$_] . '%' . $cpu_stat_values[$_]
    } 0 .. $#cpu_stat_labels;
}
