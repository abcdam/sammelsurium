...
sub get_update {
    ...
    my ( $prev_rx, $prev_tx )
      = @{ $self->{prev_stats} || [ $self->_get_wifi_bytes() ] };
    ...
    my $rx_value
      = $self->colorify_entry( $rx_formatted, $self->{color}{value} );
    my $tx_value
      = $self->colorify_entry( $tx_formatted, $self->{color}{value} );
...
sub _get_wifi_bytes {
    ...
    open my $net_dev_fh, '<', '/proc/net/dev'
