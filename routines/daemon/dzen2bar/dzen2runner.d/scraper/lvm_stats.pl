...
sub get_update {
    ...
    for my $vol ( sort keys %current_stats ) {
        ...
        my ( $prev_read, $prev_write )
          = @{ $self->{prev_stats}{$vol} || [ 0, 0 ] };

        my ( $r_speed, $r_unit )
          = $self->_format_speed(
            ( $reads - $prev_read ) / $self->{interval} );
        my ( $w_speed, $w_unit )
          = $self->_format_speed(
            ( $writes - $prev_write ) / $self->{interval}, 1 );

        my @results
          = ( $self->colorify_entry( $vol, $self->{color}{label} ) );
        push @results,
