...
_set_status_outcome() {
    printf '[%s]' "$(_whitespace_pad "$(colorify "$1" "$2" 'b')" 6)"
...
_status_info()  { _set_status_outcome 'INFO'    'lb'    "$1"; }
...
# same format as status_line(), but generates the whole entry atomically. Useful for state transitions
status_info() {
...
throw() {
    ramble "$(colorify "[FATAL] $1" r b)" >&2
    exit $(( ${2-1} )) ## Exit code at $2, or 1 by default
}

# general purpose printers
_shared_rambling_fmt() { ramble "$1" "[$(colorify "$2" "$3")]"; }

yell()  { _shared_rambling_fmt "$1" 'crit' 'r';  }
bless() { _shared_rambling_fmt "$1" ' ok ' 'lg'; }
hint()  { _shared_rambling_fmt "$1" 'hint' 'y';  }
